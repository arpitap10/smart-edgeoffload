...
* Uses *damped-trend* double exponential smoothing (Holt, 1957 + Gardner 1985)
  with α=0.35, β=0.25, φ=0.88.  The damping factor φ prevents the trend
...
class CongestionPredictor:
    ...
    ALPHA = 0.35   # level smoothing  (higher → more responsive to recent data)
    BETA  = 0.25   # trend smoothing
    PHI   = 0.88   # damping factor   (< 1 prevents runaway trend extrapolation)
    CLIP  = 1.35   # max prediction = CLIP × series max (guards against outliers)
    ...
    def _hw_statsmodels(self, series: list[float]) -> float | None:
        """Statsmodels ExponentialSmoothing with damped trend — optimised fit."""
        try:
    ...
    def predict_congestion(self, queue_series: list[float], silent: bool = False) -> float:
        ...
        Parameters
        ----------
        queue_series : recent backlog observations (seconds)
        silent       : suppress per-call logging

        Returns
        -------
        Predicted backlog >= 0, clipped to CLIP × series maximum.
        """
        if not queue_series:
            return 0.0
        ...
        # flat series — no trend to exploit
        ...
        # short-window linear trend (capped to avoid runaway)
        ...
        if len(queue_series) >= 3:
            ...
            slope = max(-0.4, min(0.4, slope))   # cap: at most ±0.4 s per slot
        trend_val = last + slope

        # blend: 55 % HW + 30 % linear trend + 15 % last observation
    ...
    def rolling_mae(self, series: list[float]) -> dict:
        ...
        Note: for mean-reverting queue series the persistence baseline is
        a strong competitor on raw MAE.  The value of HW is *early congestion
        detection* — see the paper for a fuller discussion.
        """
